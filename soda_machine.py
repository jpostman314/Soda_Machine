...
class SodaMachine:
    ...
    def run_transaction(self, customer):
        ...
        selected_soda_price = self.get_inventory_soda(selected_soda_name)

        customer_payment = customer.gather_coins_from_wallet(selected_soda_name, selected_soda_price)

        self.calculate_transaction(customer_payment, selected_soda_name, selected_soda_price, customer)

        user_interface.output_text("Transaction complete")

    def calculate_transaction(self, customer_payment, selected_soda_name, selected_soda_price, customer):
        ...
        if total_payment_value > selected_soda_price:
            ...
            if customer_change is None:
                ...
                self.return_inventory(selected_soda_name)
            else:
                self.deposit_coins_into_register(customer_payment)
                customer.add_coins_to_wallet(customer_change)
                customer.add_can_to_backpack(selected_soda_name)
                user_interface.end_message(selected_soda_name, change_value)
        elif total_payment_value == selected_soda_price:
            self.deposit_coins_into_register(customer_payment)
            customer.add_can_to_backpack(selected_soda_name)
            user_interface.end_message(selected_soda_name, 0)
        else:
    ...
    def get_inventory_soda(self, selected_soda_name):
        ...
        for can in self.inventory:
            if can.name == selected_soda_name:
                ...
                return can.price
        return self.selected_soda

    def return_inventory(self, selected_soda_name):
        """Re-adds a remove can back to inventory upon unsuccessful purchase attempt"""
        conf_bool = False
        while conf_bool == False:
            for can in self.inventory:
                if can.name == selected_soda_name:
                    self.inventory.append(can)
                    conf_bool = True
                return True
